...
model_fitting <- function(dataset = aio_mean, mean = T, return_models = F, sentinel1 = F){
  ...
  if(sentinel1 == F){
    ...
    for(tree in unique(aio$tree_id)){
      ...
      #get doys; as not all trees are present in some of the orthomosaics, the doylist is extracted tree-specific
      doylist <- unique(aio$doy[aio$tree_id == tree])
      doystart <- head(doylist,1)
      doyend <- tail(doylist,1)
      ...
      if(length(ndvidat) == 0){
        print(paste('not data for platform ',platform, ', tree ',tree))
        model_fitting_out <- rbind(model_fitting_out, data.frame(platform = "Sentinel-1",
                                                                 tree_id = tree,
                                                                 SOS = NA,
                                                                 MOS = NA,
                                                                 EOS = NA,
        ...
        models[[countvar]] <- list(platform = "Sentinel-1", tree_id = tree, ndvidat = NA, doylist = NA, model = NA)
        countvar <- countvar+1
      } else {
        # see, if model can be calculated; if not, move on
        ...
        if(is(tt,'warning')){
          print(paste('warning at platform ',"Sentinel-1", ', tree ',tree))
          model_fitting_out <- rbind(model_fitting_out, data.frame(platform = "Sentinel-1",
                                                                   tree_id = tree,
                                                                   SOS = NA,
                                                                   MOS = NA,
          ...
          models[[countvar]] <- list(platform = "Sentinel-1", tree_id = tree, ndvidat = ndvidat, doylist = doylist, model = NA)
          countvar <- countvar+1
        } else if(is(tt,'error')){
          print(paste('error at platform ',"Sentinel-1", ', tree ',tree))
          model_fitting_out <- rbind(model_fitting_out, data.frame(platform = "Sentinel-1",
                                                                   tree_id = tree,
                                                                   SOS = NA,
                                                                   MOS = NA,
          ...
          models[[countvar]] <- list(platform = "Sentinel-1", tree_id = tree, ndvidat = ndvidat, doylist = doylist, model = fitmodel)
          countvar <- countvar+1
        }
      }
    } 
